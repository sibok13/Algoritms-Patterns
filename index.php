...
/* Фрон контроллером является класс RouteCollection(), который объеденяет в себе все маршруты для взаимодействия с классами 
MainController, ProductController, OrderController, UserController */

/* 2. Найти в проекте паттерн Registry и объяснить, почему он был применён. */

/* Паттерн находится в architecture/app/framework/Registry.php Он используется для доступа к конфигурационным данным и доступа
к инстансу UrlGenerator, нужного многим другим объектам. */
    ...
    public function getById(int $id): ?Entity\User
    {
        if(isset(self::$identityMap[$id])){
            ...
            foreach ($this->getDataFromSource(['id' => $id]) as $user) {
                $resultUser = $this->createUser($user);
                self::$identityMap[$id] = $resultUser;
                return $resultUser;
            }
            return null;
        }
